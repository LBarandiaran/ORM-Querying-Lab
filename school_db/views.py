...
def problem_one(request):
    ...
    a_and_b_students = Student.objects.filter(gpa__gt=3.0).order_by('-gpa')
...
def problem_two(request):
    grizzled_instructors = Instructor.objects.filter(hire_date__year__lt=2010).order_by('hire_date')

    for instructor in grizzled_instructors:
...
def bonus_problem(request):
    ...
    daytona_course_count = Course.objects.filter(instructor__contains=1).count()
    robinson_course_count = Course.objects.filter(instructor__contains=2).count()
    delacruz_course_count = Course.objects.filter(instructor__contains=3).count()
    li_course_count = Course.objects.filter(instructor__contains=4).count()
    grimm_course_count = Course.objects.filter(instructor__contains=5).count()
    baskshi_course_count = Course.objects.filter(instructor__contains=6).count()
